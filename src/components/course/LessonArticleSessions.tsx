...
import type { ArticleSession, ArticleSubSession } from "@/lib/lesson-sessions";
...
function SubSessionBody({ sub }: { sub: ArticleSubSession }) {
  if (sub.html) {
    return (
      <div
  ...
  if (!sub.blocks.length) {
    return <p className="text-sm text-muted-foreground italic">Contenu à venir.</p>;
  }
