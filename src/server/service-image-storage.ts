...
export async function uploadServiceImage(params: {
  ...
  const slug = params.serviceSlug.trim().toLowerCase();
  if (!slug) {
    return { ok: false, reason: "Service invalide" };
  }
  ...
  if (error) {
    if (error.message.includes("Bucket not found") || error.message.includes("does not exist")) {
      return {
        ok: false,
        reason: "Bucket service-images manquant — exécutez supabase/service_images_storage.sql",
      };
    }
    console.error("[BelKou] service image upload:", error.message);
